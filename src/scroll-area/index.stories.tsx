      ...
      <div className="space-y-4">
        ...
        <div className="text-sm">
          {Array.from({ length: 20 }).map((_, i) => (
            <div key={i} className="mb-2 rounded bg-white p-2">
              <p className="text-blue-800">Item {i + 1}</p>
              <p className="text-blue-600">This is a custom styled scroll area with a blue theme.</p>
            </div>
          ))}
        </div>
      </div>
            ...
            <div key={i} className="flex items-center gap-4 rounded-lg border p-3">
              <div className="grid gap-1">
                ...
                <p className="text-muted-foreground text-sm">
                  This is a notification message that might be quite long and need to be truncated or wrapped.
                </p>
              </div>
            </div>
          ))}
